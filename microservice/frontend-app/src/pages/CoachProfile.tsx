...
import getCoachSlots from "../services/getCoachSlots";
...
const CoachProfile: React.FC = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      if (selectedDate) {
        const response = await getCoachSlots(coachProfile?.id, formatDate1(selectedDate))
        setTimeArr(response);
      }
    }
    fetch()
  ...
  const handleBookWorkout = async () => {
    ...
    const request = {
      ...
      timeSlot: selectedSlotTime.split(" -")[0],
    };
        ...
        >
          ...
          <div className="flex flex-col p-2 gap-2">
            <div className="flex justify-between gap-10">
              <div>
                <h1 className="font-[500] text-xl">{`${coachProfile.name}`}</h1>
              ...
              <div className="pt-2">
                <Button
                  ...
                  onSubmit={() => {
                    ...
                    if (!selectedDate) {
                      alert("Please select a date to book a workout");
                      return;
                    }
                    setBlurDialog(true)
                  }
        ...
        >
            ...
            >
              ...
              <div className="flex-1 flex flex-col">
                ...
                <div className="max-h-[15em] overflow-auto">
                  ...
                  {timeArr.length > 0 ? timeArr.map(((time, i) => (
                    <div onClick={() => {
                      setSelectedSlotTime(time);
                      setSelectedSlotIndex(i);
                  ...
                  ))) : <div className="flex justify-center py-3">No Slots Available</div>}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-5">
            <Button
              ...
              onSubmit={() => {
                if (!selectedSlotTime) {
                  alert("Please select a valid date and time slot to book a workout");
                  return;
                }
                setBlurDialog(true)
              }}
