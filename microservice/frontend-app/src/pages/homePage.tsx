...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchdata = async () => {
      ...
      setFilterCoaches([
        "All",
        ...response.map((coach: CoachBackendInterface) => coach.name),
      ]);
  ...
  useEffect(() => {
    if (coach) {
      const selectedCoach = allCoaches.find((item) => item.name === coach);
  ...
  const onSubmit = async () => {
    setWorkoutLoad(true);
    console.log({ sport, time, date, coach: coachId });
    const response = await filterWorkout({ sport, time, date, coach: coachId });
    if (response.error) {
      console.log("");
    } else {
  ...
  const bookingWorkout = async () => {
    ...
    console.log(
      {
        coachId: selectedCoach?.coachName.id || "",
        date: date || "",
        timeSlot: time || "",
      },
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-between my-6">
                  ...
                  <div className="text-[#4B5563] flex flex-col justify-between max-h-[4.5rem]">
                    <div>
                      <div className=" font-semibold text-[0.95rem]">
                        {`${selectedCoach?.coachName.name}`}
                      </div>
                      <div className=" font-light text-[0.75rem]">
                        {selectedCoach?.coachName.title}
                      </div>
                    </div>
                    <div className="flex gap-1 font-light text-[0.8rem]">
                      {selectedCoach?.coachName.ratings}
                      <Star />
                    </div>
                  </div>
                </div>
                <div className="text-[0.8rem] space-y-1.5">
                  <div className="flex gap-2 text-[#4B5563] font-light">
                    ...
                    {selectedCoach?.coachName.specialization}
                  </div>
                  <div className="flex gap-2 text-[#4B5563] font-light">
                    <Clock />
          ...
          <div className="px-2 sm:px-8 my-10 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-5 ">
              {resultArr.map((item: CoachInterface) => (
                <CoachCard
                  name={`${item.coachName.name}`}
                  title={item.coachName.title}
                  rating={item.coachName.ratings}
                  summary={item.coachName.shortSummary}
                  type={item.sport}
                  time={item.time}
                  date={item.date}
                  timeArr={item.freeSlots}
                  coachId={item.coachName.id}
                  onSubmit={() => {
